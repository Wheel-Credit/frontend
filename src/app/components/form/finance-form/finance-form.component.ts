...
export class FinanceFormComponent {
  ...
  Algorithm() {
    ...
    //TEA
    var TEA = this.GetTEA(this.smartPaymentAux.interestType, this.smartPaymentAux.interestRate!, this.smartPaymentAux.capitalization);
    console.log("TEA: " + TEA)
    ...
    var Number_of_Installments_per_year = this.GetNumInstallments(this.smartPaymentAux.paymentFrequency!);
    console.log("Number of Installments per year: " + Number_of_Installments_per_year)
    ...
    //Cuota Inicial
    var InitialInstallment = this.GetInitialInstallment(this.smartPaymentAux.sellingPriceAsset!, this.smartPaymentAux.initialInstallment!);
    console.log("InitialInstallment: " + InitialInstallment)

    //Cuota Final
    var FinalInstallment = this.getFinalInstallment(this.smartPaymentAux.sellingPriceAsset!, this.smartPaymentAux.finalInstallment!);
    console.log("FinalInstallment: " + FinalInstallment)

    //Monto del Prestamo
    var LoanAmount = this.getLoanAmount(this.smartPaymentAux.sellingPriceAsset!, InitialInstallment, 
      this.smartPaymentAux.notaryCosts!, this.smartPaymentAux.notaryCostsType, this.smartPaymentAux.registrationCosts!, 
      this.smartPaymentAux.registrationCostsType, this.smartPaymentAux.appraisal!, this.smartPaymentAux.appraisalType, 
      this.smartPaymentAux.studyCommission!, this.smartPaymentAux.studyCommissionType, this.smartPaymentAux.activationCommission!, 
      this.smartPaymentAux.activationCommissionType);
    console.log("LoanAmount: " + LoanAmount)
    
    //Saldo a financiar con cuotas
    var BalanceToFinanceWithInstallments = this.getBalanceToFinanceWithInstallments(LoanAmount, FinalInstallment,TEP, this.smartPaymentAux.lifeInsurance!, TotalInstallments!);
    console.log("BalanceToFinanceWithInstallments: " + BalanceToFinanceWithInstallments)
  
    //% de Seguro desgrav. per.
    var PercentageOfReliefInsurance = (this.smartPaymentAux.lifeInsurance!/100) * this.smartPaymentAux.paymentFrequency!/30;
    console.log("PercentageOfReliefInsurance: " + PercentageOfReliefInsurance)

    //Seguro riesgo
    var RiskInsurance = (this.smartPaymentAux.riskInsurance!/100) * this.smartPaymentAux.sellingPriceAsset!/Number_of_Installments_per_year;
    ...
    ////Pruebas
    //Falta Comprobar
    console.log("Calcular pago: " + this.CalcularPago(TEP, PercentageOfReliefInsurance, 36, 9015.99));
  }
  ...
  GetTEA(interestType: String, interestRate: number, capitalization: String) {
    ...
    return Math.pow(1 + (interestRatePorcent / (360)), this.number_of_days_per_year/Captitalization_days) - 1;
  }

  getTEP(TEA: number, paymentFrequency: number) {
    return Math.pow(1 + TEA, paymentFrequency/this.number_of_days_per_year) - 1;
  }
  ...
  getLoanAmount(sellingPriceAsset: number, initialInstallment: number, notaryCosts: number, notaryCostsType: String, registrationCosts: number, registrationCostsType: String, appraisal: number, appraisalType: String, studyCommission: number, studyCommissionType: String, activationCommission: number, activationCommissionType: String){
  ...
  //Saldo a financiar con cuotas
  getBalanceToFinanceWithInstallments(LoanAmount: number, finalInstallment: number, TEP: number, lifeInsurance: number, TotalInstallments: number){
    return LoanAmount-finalInstallment/(Math.pow((1+TEP+(lifeInsurance/100)), TotalInstallments+1));
  }
  ...
  getTable(finalInstallment: number, TEP: number, lifeInsurance: number, TotalInstallments: number, PercentageOfReliefInsurance: number, BalanceToFinanceWithInstallments: number, RiskInsurance: number){
    ...
    for(var i = 0; i < NumberofInstallments; i++){
      //Saldo inicial Cuota Final
      if(i == 0){
        InitialInstallmentFinalInstallment[i] = finalInstallment/Math.pow(1+TEP+(lifeInsurance/100),TotalInstallments+1);
      }
      else{
        InitialInstallmentFinalInstallment[i] = FinalBalanceFinalInstallment[i-1];
      }

      //Interes Cuota Final
      FinalInstallmentInterest[i] = -InitialInstallmentFinalInstallment[i] * TEP;

      //Amort. Couta Final
      if(i == NumberofInstallments-1){
        FinalInstallmentAmortization[i] = -InitialInstallmentFinalInstallment[i]+FinalInstallmentInterest[i]+CreditInsuranceFinalInstallment[i];
      }

      //Seguro desgav. Cuota Final
      CreditInsuranceFinalInstallment[i] = -InitialInstallmentFinalInstallment[i]*(PercentageOfReliefInsurance);

      //Saldo Final Cuota Final
      FinalBalanceFinalInstallment[i] = InitialInstallmentFinalInstallment[i]-FinalInstallmentInterest[i]-CreditInsuranceFinalInstallment[i]+FinalInstallmentAmortization[i];
      ...
      if(i<=NumberofInstallments-1){
        if(0){ // En caso que sea un Plazo de Gracia Total o Parcial
          ...
          Amortization[i] = Installments[i] - Interest[i] - InsuranceCreditInstallment[i];
        }
      }
      else{
        Amortization[i] = 0;
      ...
      //Seguro desg. Cuota
      InsuranceCreditInstallment[i] = -InitialBalanceInstallment[i]*PercentageOfReliefInsurance;
      ...
      //Gastos Adm.
      if(i<=NumberofInstallments+1){ // +1?? Según el excel es +1
        AdministrativeExpensesTable[i] = this.smartPaymentAux.administrativeExpenses!;
      }
      ...
      if(0){ // Si es un Plazo de Gracia Total
        ...
        FinalBalanceInstallment[i] = InitialBalanceInstallment[i] + Amortization[i];
      }

      //Flujo
      if(0){ // Si es un Plazo de Gracia Total o Parcial
        if(i==NumberofInstallments-1){
          Flow[i] = -Installments[i] + RiskInsuranceTable[i] + GPSTable[i] + ShippingCostsTable[i] + AdministrativeExpensesTable[i]+InsuranceCreditInstallment[i]+FinalInstallmentAmortization[i];
        }
        else{
          Flow[i] = -Installments[i] + RiskInsuranceTable[i] + GPSTable[i] + ShippingCostsTable[i] + AdministrativeExpensesTable[i]+InsuranceCreditInstallment[i];
        }
      }
      else{
        if(i==NumberofInstallments-1){
          Flow[i] = -Installments[i] + RiskInsuranceTable[i] + GPSTable[i] + ShippingCostsTable[i] + AdministrativeExpensesTable[i]+FinalInstallmentAmortization[i];
        }
        else{
          Flow[i] = -Installments[i] + RiskInsuranceTable[i] + GPSTable[i] + ShippingCostsTable[i] + AdministrativeExpensesTable[i]
        }
      }
    }
  }
  ...
  CalcularPago(TEP: number, PercentageOfReliefInsurance: number, NumberofInstallments: number, InitialBalanceInstallment: number){
    const pagoMensual= PercentageOfReliefInsurance * (TEP * Math.pow(1+TEP, NumberofInstallments)) / (Math.pow(1+TEP, NumberofInstallments)-1);
